...
class TraceAnalyser:
    ...
    def create_annotated_task_graph(self) -> nx.DiGraph:
        ...
        for event in tqdm.tqdm(self._database,
                               desc="Creating annotated task graph"):
            if event.name == "Compute":
                ...
                # Add the task as a node to the graph and store task
        ...
        for node in graph.nodes:
            input_data_size = sum([graph.nodes[dependency].get('size', 0)
                                   for dependency in
                                   graph.predecessors(node)])

            # Set the input_data_size attribute for the current node
            graph.nodes[node]['input_data_size'] = input_data_size
            graph.nodes[node]["throughput"] = \
                input_data_size / graph.nodes[node].get("duration", 1)
    ...
    def per_function_bottleneck(self):
        ...
        # Dictionary to store mean gpu_utilization and gpu_memory_used
        # per task_key
        task_resources = defaultdict(lambda: {'gpu_utilization': [],
                                              'gpu_memory_used': []})
        # Dictionary mapping name to keys
        task_name_keys = defaultdict(lambda: defaultdict(list))

        # Iterate over the traces to calculate task durations per thread_id
        for event in tqdm.tqdm(self._database,
                               desc="[function_bottleneck] Analysing traces"):
            if event.name == "Compute":
                task_key = event.args['name']
                task_duration = event.duration
                thread_id = event.thread_id
                process_id = event.process_id
                task_name_keys[(process_id, thread_id)][task_key].append(
                    event.args['key'])
                task_durations[(process_id, thread_id)][task_key] += \
                    task_duration
                ...
                # Find the corresponding task for the resource event based
                # on timestamp
                task_key = None
                for task_event in self._database:
                    if (task_event.name == "Compute" and
                        task_event.timestamp <= event_timestamp <=
                            (task_event.timestamp + task_event.duration)):
                        task_key = task_event.args['name']
                        break

                if task_key is not None:
                    task_resources[task_key]['gpu_utilization'].append(
                        gpu_utilization)
                    task_resources[task_key]['gpu_memory_used'].append(
                        gpu_memory_used)

        # Create a list of dictionaries to store data for the DataFrame
        data = []
        for (process_id, thread_id), durations in tqdm.tqdm(
                task_durations.items(),
                desc="[function_bottleneck] Creating dataframe"):
            total_duration = sum(durations.values())
            for task_key, duration in durations.items():
                percentage = (duration / total_duration) * 100
                gpu_utilization_values = \
                    task_resources[task_key]['gpu_utilization']
                gpu_memory_used_values = \
                    task_resources[task_key]['gpu_memory_used']
                num_tasks = \
                    len(task_name_keys[(process_id, thread_id)][task_key])
                mean_data_size = 0
                mean_throughput = 0
                count = 0
                for name_key in \
                        task_name_keys[(process_id, thread_id)][task_key]:
                ...
                mean_gpu_utilization = \
                    sum(gpu_utilization_values) / \
                    len(gpu_utilization_values) if \
                    len(gpu_utilization_values) > 0 else 0
                mean_gpu_memory_used = \
                    sum(gpu_memory_used_values) / \
                    len(gpu_memory_used_values) if \
                    len(gpu_memory_used_values) > 0 else 0
                data.append({
                    'Host': process_id,
    ...
    def per_worker_task_balance(self):
        ...
        # Iterate over the traces to calculate the number of tasks per worker
        # at each timestamp
        for event in tqdm.tqdm(self._database,
                               desc="[task_balance] Analysing traces"):
        ...
        for timestamp in tqdm.tqdm(timestamps,
                                   desc="[task_balance] Creating dataframe"):
        ...
        # Create a Pandas DataFrame from the mean_tasks_per_interval dict
        df = pd.DataFrame.from_dict(mean_tasks_per_interval, orient='index')

        df = df.reindex(sorted(df.columns), axis=1)

        # Fill missing values with 0 (if a thread didn't have any tasks in a
        # specific interval)
        ...
        df.rename(columns={'index': 'Time Interval (seconds from begin)'},
                  inplace=True)
        # df["Time Interval"] = df["Time Interval"].apply(
        #    lambda x: x + start_time)

        # Print the DataFrame showing the mean number of tasks per thread in
        # each time interval
    ...
    def per_task_bottleneck(self):
        ...
        # Iterate over the traces to calculate task durations per thread_id
        for event in tqdm.tqdm(self._database,
                               desc="[task_bottleneck] Analysing traces"):
            if event.name == "Compute":
                task_key = event.args['key']
                task_duration = event.duration
                thread_id = event.thread_id
        ...
        for (process_id, thread_id), durations in tqdm.tqdm(
                task_durations.items(),
                desc="[task_bottleneck] Creating dataframe"):
            total_duration = sum(durations.values())
            for task_key, duration in durations.items():
                percentage = (duration / total_duration) * 100
                # gpu_utilization_values = \
                #     task_resources[task_key]['gpu_utilization']  # Unused
                # gpu_memory_used_values = \
                #     task_resources[task_key]['gpu_memory_used']  # Unused
                data.append({
                    'Host': process_id,
                    "GPU": thread_id.split("-")[-1],
                    'Task Key': task_key,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--databases", type=str, nargs="+",
                        help="The databases to analyse", required=True)
    parser.add_argument("-o", "--output", type=str,
                        help="The output directory, to save output analysis. "
                             "If None, print only in screen", required=False)
    parser.add_argument("-a", "--analyses", type=str, nargs="+",
                        help="The analyses to perform (if None, perform all)",
                        required=False)
