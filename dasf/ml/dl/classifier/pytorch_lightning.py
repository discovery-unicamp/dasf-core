...
def trainer(model, epochs, accel, strategy, devices, train, val, metadata, batch_size, auto_scale_batch_size, ngpus):
    ...
    trainer = pl.Trainer(max_epochs=epochs, accelerator=accel, strategy=strategy, gpus=ngpus, plugins=[DaskClusterEnvironment(metadata)],
                         devices=devices, num_nodes=nodes, auto_scale_batch_size=auto_scale_batch_size)
...
class Trainer(Operator):
    ...
    def __run_clustered(self, model, train, val, batch_size, auto_scale_batch_size=None):
        ...
        for worker in all_workers:
            ...
            futures = self.client.submit(trainer, model, self.epochs, self.accel, self.strategy,
