...
class TestDownloadFunctions(unittest.TestCase):
    ...
    def test_download_file_basic(self, mock_exists, mock_gdown, mock_notebook):
        ...
        mock_gdown.assert_called_once_with("http://example.com/file.txt", output=os.path.abspath(os.path.join(os.getcwd(), "test_file.txt")))
    ...
    @patch('os.path.exists', return_value=False)
    @patch('os.makedirs')
    def test_download_file_with_directory(self, mock_makedirs, mock_exists, mock_gdown, mock_notebook):
        mock_gdown.return_value = "test_file.txt"
        
        result = download_file("http://example.com/file.txt", "test_file.txt", "/tmp/test")
    ...
    def test_download_file_already_exists(self, mock_exists, mock_gdown, mock_notebook):
        result = download_file("http://example.com/file.txt", "existing_file.txt")
    ...
    @patch('gdown.download')
    @patch('os.path.exists', return_value=False)
    def test_download_file_with_notebook(self, mock_exists, mock_gdown, mock_progressbar, mock_notebook):
        mock_pbar = Mock()
    ...
    def test_download_file_from_gdrive(self):
        with patch('dasf.utils.funcs.download_file') as mock_download:
            ...
            expected_url = "https://drive.google.com/uc?export=download&confirm=9iBg&id=file123"
            mock_download.assert_called_once_with(expected_url, filename="test.txt", directory="/tmp")
...
class TestProgressBarIntegration(unittest.TestCase):
    ...
    def test_notebook_progress_bar_show(self, mock_display, mock_hbox, mock_label, mock_float_progress):
        from dasf.utils.funcs import NotebookProgressBar
