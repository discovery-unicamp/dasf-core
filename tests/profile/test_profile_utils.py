...
from dasf.profile.profiler import EventDatabase, EventProfiler
...
class TestRegisterDefaultProfiler(unittest.TestCase):
    ...
    def test_register_default_profiler_with_defaults(self, mock_print, mock_atexit, 
        ...
        mock_worker_plugin.assert_called_once_with(name="default-1234567890-TracePlugin")
        mock_resource_monitor.assert_called_once_with(name="default-1234567890-ResourceMonitor")
        
        self.mock_pipeline.register_plugin.assert_called_once_with(mock_worker_instance)
        
        mock_atexit.assert_called_once()
        
        mock_print.assert_any_call("Registered worker plugin: default-1234567890-TracePlugin")
        mock_print.assert_any_call("Registered resource plugin: default-1234567890-ResourceMonitor")
    ...
    @patch('dasf.profile.utils.atexit.register')
    @patch('builtins.print')
    def test_register_default_profiler_with_custom_name(self, mock_print, mock_atexit,
                                                       mock_resource_monitor, mock_worker_plugin,
                                                       mock_time):
        mock_worker_instance = Mock()
        mock_resource_instance = Mock()
        ...
        mock_worker_plugin.assert_called_once_with(name="custom_profiler-9876543210-TracePlugin")
        mock_resource_monitor.assert_called_once_with(name="custom_profiler-9876543210-ResourceMonitor")
        
        mock_print.assert_any_call("Registered worker plugin: custom_profiler-9876543210-TracePlugin")
        mock_print.assert_any_call("Registered resource plugin: custom_profiler-9876543210-ResourceMonitor")
    ...
    @patch('dasf.profile.utils.atexit.register')
    @patch('builtins.print')
    def test_register_default_profiler_without_time_suffix(self, mock_print, mock_atexit,
                                                          mock_resource_monitor, mock_worker_plugin,
                                                          mock_time):
        mock_worker_instance = Mock()
        mock_resource_instance = Mock()
        ...
        register_default_profiler(self.mock_pipeline, name="no_time", add_time_suffix=False)
    ...
    @patch('dasf.profile.utils.atexit.register')
    @patch('builtins.print')
    def test_register_default_profiler_with_nvtx(self, mock_print, mock_atexit,
                                                mock_gpu_plugin, mock_resource_monitor, 
                                                mock_worker_plugin, mock_time):
        mock_worker_instance = Mock()
        mock_resource_instance = Mock()
        ...
        mock_worker_plugin.assert_called_once_with(name="default-2222222222-TracePlugin")
        mock_resource_monitor.assert_called_once_with(name="default-2222222222-ResourceMonitor")
        ...
        mock_print.assert_any_call("Registered worker plugin: default-2222222222-TracePlugin")
        mock_print.assert_any_call("Registered resource plugin: default-2222222222-ResourceMonitor")
        mock_print.assert_any_call("Registered GPU annotation plugin (NVTX)")

    @patch('dasf.profile.utils.time.time', return_value=3333333333)
    @patch('dasf.profile.utils.WorkerTaskPlugin')
    @patch('dasf.profile.utils.ResourceMonitor')
    @patch('dasf.profile.utils.atexit.register')
    def test_register_default_profiler_atexit_close_function(self, mock_atexit,
                                                           mock_resource_monitor, mock_worker_plugin,
                                                           mock_time):
        mock_worker_instance = Mock()
        mock_resource_instance = Mock()
