...
class TestTraceAnalyser(unittest.TestCase):
    ...
    def test_per_function_bottleneck(self, mock_tqdm):
        ...
        if len(df) > 1:
            ...
            self.assertTrue(all(durations[i] >= durations[i+1]
    ...
    def test_per_worker_task_balance(self, mock_tqdm):
        ...
        if len(df) > 1:
            ...
            self.assertTrue(all(intervals[i] <= intervals[i+1]
...
class TestMainFunction(unittest.TestCase):
    ...
    def test_main_function_with_all_analyses(self,
                                             mock_print,
                                             mock_analyser_class,
                                             mock_set_option):
        mock_analyser = Mock()
    ...
    @patch('dasf.profile.analysis.TraceAnalyser')
    @patch('builtins.print')
    def test_main_function_with_specific_analysis(self,
                                                  mock_print,
                                                  mock_analyser_class,
                                                  mock_set_option):
        mock_analyser = Mock()
    ...
    def test_main_function_with_output_directory(self,
                                                 mock_path,
                                                 mock_analyser_class,
                                                 mock_set_option):
        mock_analyser = Mock()
